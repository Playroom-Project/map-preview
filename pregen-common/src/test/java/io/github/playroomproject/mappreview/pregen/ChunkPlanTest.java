...
class ChunkPlanTest {
    @Test void sparseSpiralYieldsWithoutMistakingAScanBudgetForExhaustion() {
        var area = new ChunkArea() {
            @Override public ChunkBounds bounds() { return new ChunkBounds(-1_000_000, -1_000_000, 1_000_000, 1_000_000); }
            @Override public long totalChunks() { return 1; }
            @Override public List<ChunkSpan> rowSpans(int z) { return z == 1_000_000 ? List.of(new ChunkSpan(1_000_000, 1_000_000)) : List.of(); }
            @Override public io.github.playroomproject.mappreview.core.cache.Fingerprint fingerprint() {
                return io.github.playroomproject.mappreview.core.cache.Fingerprint.builder().add("sparse-test").finish();
            }
        };
        var cursor = new ChunkPlan(area, ChunkPlan.Traversal.SPIRAL).cursor(0);
        assertNull(cursor.poll(128));
        assertFalse(cursor.exhausted());
        assertEquals(128, cursor.offset());
    }

    @ParameterizedTest @EnumSource(ChunkPlan.Traversal.class)
    void boundedPollingPreservesEveryAcceptedChunk(ChunkPlan.Traversal traversal) {
        var plan = new ChunkPlan(ChunkAreas.circle(-3, -7, 19), traversal);
        var cursor = plan.cursor(0);
        var positions = new HashSet<ChunkPos>();
        while (!cursor.exhausted()) {
            var position = cursor.poll(1);
            if (position != null) { assertTrue(positions.add(position)); }
        }
        assertEquals(plan.totalChunks(), positions.size());
    }
